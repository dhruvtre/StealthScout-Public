...
# 1. Query Stealth Founder Table - query_stealth_founder_table
# 2. Form and Button for Search - st.form(key="search_form"), search_submitted
# 3. Display Profile Card and Parse Duration - display_profile_card, parse_duration
# 4. Parent Function executing View - search_db_view
# ------------------------------

def search_db_view(supabase_client, list_of_search_companies):
    ...
    with st.form(key="search_form"):
        ...
        st.markdown('<div class="section-text">Filter by Profile Status</div>', unsafe_allow_html=True)
        filter_profile_status = st.multiselect(
            "Choose profile status",
            ["Stealth", "Building in Public", "Recently Quit"],
            placeholder="Select one or more status types",
            label_visibility="collapsed"
        )
    ...
    if search_submitted:
        ...
        logger.info(f"Profile status filters applied: {filter_profile_status if filter_profile_status else 'None'}")
        ...
        with st.spinner("Searching for profiles..."):
            ...
            for company_name in past_company_name:
                list_of_profiles_retrieved = query_stealth_founder_table(
                    supabase_client, 
                    company_name,
